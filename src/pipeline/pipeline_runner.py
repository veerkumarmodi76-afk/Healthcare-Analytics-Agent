...
class PipelineRunner:
    ...
    def run(self) -> bool:
        ...
        try:
            ...
            self._execute_stage(
                ...
                func=lambda: self._run_underwriting(mode),
                exception_cls=UnderwritingException,
            )

            self._execute_stage(
                stage_name="Pricing",
                func=lambda: self._run_pricing(mode),
                exception_cls=PricingException,
            )

            self._execute_stage(
                stage_name="Lapse",
                func=lambda: self._run_lapse(mode),
    ...
    def _execute_stage(
        ...
        current_stage = (
            [stage[0] for stage in self.STAGES].index(stage_name) + 1
        )
    ...
    def _notify_progress(self):
        """
        Notify the Streamlit UI whenever pipeline state changes.
        """
